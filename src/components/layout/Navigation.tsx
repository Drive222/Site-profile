  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border/70 bg-background/75 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
        ...
        >
          <span className="block text-base font-semibold text-text">{t('hero.name')}</span>
          <span className="block text-xs uppercase tracking-[0.18em] text-muted">{t('hero.profession')}</span>
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface/80 text-text lg:hidden"
      ...
      {isMobileNavOpen ? (
        <nav className="border-t border-border/70 bg-background/95 px-5 py-4 backdrop-blur-xl lg:hidden" aria-label={t('nav.ariaLabel')}>
